...
# Switch to On-demand capacity on existing dynamo table
- dynamodb_table:
    name: my-table
    region: us-east-1
...
try:
    from ansible.module_utils.ec2 import ansible_dict_to_boto3_tag_list, ec2_argument_spec
...
def update_dynamo_table(
    ...
    if not check_mode and kwargs:
        client.update_table(**kwargs)
        change_state.update(
            {
                'changed': True,
                'global_indexes_updates': set_global_indexes or []
            }
        )
